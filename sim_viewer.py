...
def main(on_tick=None):
    ...
    # DEMO_PAUSED=1:启动即暂停,按空格才正式开始(演出时等大家站好位)
    start_paused = os.environ.get("DEMO_PAUSED", "0") == "1"
    state = {"idx": 0, "paused": start_paused, "sim_t": 0.0,
             "last": time.time(),
             "warned": set(), "loop": False, "done": False, "speed": 1.0}
    if start_paused:
        print("⏸ 已就位待命:点一下仿真窗口,按【空格】开始演出")
